...
+ (void)logc:(LinphoneLoggerSeverity) severity format:(const char *)format,... {
    ...
    va_start (args, format);
    if(severity <= LinphoneLoggerDebug) {
        ortp_logv("phemium_videocall", ORTP_DEBUG, format, args);
    } else if(severity <= LinphoneLoggerLog) {
        ortp_logv("phemium_videocall", ORTP_MESSAGE, format, args);
    } else if(severity <= LinphoneLoggerWarning) {
        ortp_logv("phemium_videocall", ORTP_WARNING, format, args);
    } else if(severity <= LinphoneLoggerError) {
        ortp_logv("phemium_videocall", ORTP_ERROR, format, args);
    } else if(severity <= LinphoneLoggerFatal) {
        ortp_logv("phemium_videocall", ORTP_FATAL, format, args);
    }
    va_end (args);
}
